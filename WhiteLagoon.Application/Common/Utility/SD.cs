...
namespace WhiteLagoon.Application.Common.Utility
{
    public static class SD
    {
        ...
        {   
            //List bookings that collide with our booking 
            List<int> bookingInDate = new();

            int finalAvailableRoomForAllNights = int.MaxValue;
            var roomsInVilla = villaNumberList.Where(x => x.VillaId == villaId).Count(); // Get total rooms in Villa

            for (int i = 0; i < nights; i++)
            {
                // Iterate through nights to get previous booking that collides with our booking and then add into bookingInDate list
